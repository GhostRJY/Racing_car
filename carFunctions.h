...
void drawPlayerCar(const GameVars &gVar, const GameStatements &gStat)
{
    ...
    if (!gStat.m_dead)
    {
        for (int i{0}; i < 4; ++i)
        {
            ...
            SetConsoleTextAttribute(consoleHandle, Color::BLUE);
...
{
    SetConsoleTextAttribute(consoleHandle, Color::RED);
...
void runGame(GameVars &gVars, GameStatements &gStat, Enemy &enemy)
{
    ...
    for (int i{0}; i < enemy.m_eCount; ++i)
    {
        if ((gVars.m_carY <= enemy.m_enemyY[i] + 3 && gVars.m_carY >= enemy.m_enemyY[i] || enemy.m_enemyY[i] >= gVars.m_carY && enemy.m_enemyY[i] <= gVars.m_carY + 3) && gVars.m_carX == enemy.m_enemyX[i])
        {
...
{
    SetConsoleTextAttribute(consoleHandle, Color::YELLOW);
    if (gStat.m_crash)
    {
        for (int i{0}; i < 4; ++i)
        {
